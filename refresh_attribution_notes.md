...
## 効果測定（2026-07-21 実測・本番BQ）
凍結CSV（翌朝取得）と再取得CSV（当日再DL＝成熟値）を**同一日付で比較**して裏取り済み。
- 7/04 商品別 店舗全体: 凍結 **63件 → 再取得 93件（+30 / +48%）**。firebag 5→10, ltr 10→19,
  carnet 1→5, dog-shoes 5→7, cartc 11→15 等10SKUが上振れ。**再取得は最新の成熟値**（キャッシュではない）。
- 洗い替えを実行した5日（7/02,04,08,12,16）店舗全体: 凍結 **217件 → BQ現在 308件（+91 / +42%）**。
  上記はUPSERT後のBQ実値＝過少計上が実際に縮小したことを確認。
- **SKU差が大きい**: `1dayp` は即転換型で成熟ほぼ無し（5→5）。逆に firebag/ltr 等は成熟大。
  → 効果はSKU/商品性質に依存。店舗全体では約4割の過少が回復。

## 再取得の実務的制約（重要）
- RMSのレポート再生成は**直近日は高速**（実測 ≤19日: 数十秒/レポート）だが、
  **古い日は極端に遅い/タイムアウト**（実測 24日=6分ポーリングしても未完了→Skip、30日=同様）。
- したがって**過去分の一括バックフィルは「古い日ほど不可」**。既に古くなった日の過少は
  この方式では回収困難。**日次で走らせて“まだ再取得できる新しいうち”に各日を毎日更新し、
  成熟を漸進的に取り込む**のが本ツールの正しい使い方（=既定14日窓）。
- `-Days` を大きくしても古い日はタイムアウトで時間を浪費するだけ（1レポート約18分）。
  **既定14**、実用上限は概ね18〜19日。

## 運用
- 定期実行タスク `RakutenAttributionRefresh`（Interactive/S4U・毎日16:30・`-Days 14`）。
  ※ `Download-All-Reports.ps1` は Windows資格情報マネージャからRMS資格情報を読むため、
    SSH直実行では error 1312 で失敗する。**必ずInteractive/S4Uのタスク経由**で実行する。
  ※ run_all（5/6/7/9/10/12/15時）とブラウザ同時実行を避けるため16:30に配置。
- 手動バックフィル: `refresh_attribution_window.ps1 -Days 14`（または `-Dates 2026/07/10,2026/07/11` 等）。
  古い日は上記制約でタイムアウトしやすい点に注意。
